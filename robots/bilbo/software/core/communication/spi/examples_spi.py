...
def example_read_sample_buffer():
    ...
    class bilbo_gyr_data(ctypes.Structure):
        _fields_ = [("x", ctypes.c_float), ("y", ctypes.c_float), ("z", ctypes.c_float)]

    class bilbo_acc_data(ctypes.Structure):
        _fields_ = [("x", ctypes.c_float), ("y", ctypes.c_float), ("z", ctypes.c_float)]

    class bilbo_sensor_data(ctypes.Structure):
        _fields_ = [("speed_left", ctypes.c_float), ("speed_right", ctypes.c_float), ("acc", bilbo_acc_data),
                    ("gyr", bilbo_gyr_data)]

    class sample_sensors(ctypes.Structure):
        _fields_ = [("sensor_data", bilbo_sensor_data)]

    class bilbo_estimation_state(ctypes.Structure):
    ...
    class sample_estimation(ctypes.Structure):
        _fields_ = [('state', bilbo_estimation_state)]

    class bilbo_control_input(ctypes.Structure):
        _fields_ = [("u1", ctypes.c_float), ("u2", ctypes.c_float)]

    class bilbo_control_output(ctypes.Structure):
        _fields_ = [("u_left", ctypes.c_float), ("u_right", ctypes.c_float)]

    class sample_control(ctypes.Structure):
        _fields_ = [('status', ctypes.c_int8), ('mode', ctypes.c_int8), ("input", bilbo_control_input),
                    ("output", bilbo_control_output), ("tick", ctypes.c_uint32)]

    class bilbo_sample(ctypes.Structure):
    ...
    samples = []
    for i in range(0, 10):
        samples.append(bilbo_sample.from_buffer_copy(data_rx_bytes[i*80:(i+1)*80]))
