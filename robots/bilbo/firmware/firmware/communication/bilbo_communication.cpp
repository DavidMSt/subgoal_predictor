/*
 * bilbo_communication.cpp
 *
 *  Created on: 12 Mar 2023
 *      Author: Dustin Lehmann
 *
 *  Description:
 *      Implementation of the BILBO communication manager. This module handles UART, SPI,
 ...
 */
BILBO_CommunicationManager *active_manager = NULL;

/**
 * @brief Default constructor for BILBO_CommunicationManager.
 *
 * Currently no additional initialization is performed in the constructor.
 */
BILBO_CommunicationManager::BILBO_CommunicationManager() {
}
 ...
 */
void BILBO_CommunicationManager::init(bilbo_communication_config_t config) {
    ...
    // ---------------------------
    bilbo_uart_comm_config_t uart_config = { .huart = this->config.huart };
    this->uart_interface.init(uart_config);

    // Register callback for UART message write events.
    this->uart_interface.registerCallback(
        BILBO_UART_COMM_CALLBACK_MSG_WRITE,
        core_utils_Callback<void, core_comm_SerialMessage*>(this,
            &BILBO_CommunicationManager::_uart_handleMsg_write_callback));

    // Register callback for UART message read events.
    this->uart_interface.registerCallback(
        BILBO_UART_COMM_CALLBACK_MSG_READ,
        core_utils_Callback<void, core_comm_SerialMessage*>(this,
            &BILBO_CommunicationManager::_uart_handleMsg_read_callback));

    // Register callback for UART function message events.
    this->uart_interface.registerCallback(
        BILBO_UART_COMM_CALLBACK_MSG_FUNC,
        core_utils_Callback<void, core_comm_SerialMessage*>(this,
            &BILBO_CommunicationManager::_uart_handleMsg_func_callback));

    // Register external interrupt callback for UART reset.
    core_utils_gpio_registerExtiCallback(
        this->config.reset_uart_exti,
        core_utils_Callback<void, void>(this, &BILBO_CommunicationManager::resetUART));
    ...
    this->spi_interface.callbacks.trajectory_received.registerFunction(this,
    		&BILBO_CommunicationManager::_spi_rxTrajectory_callback);


    this->spi_interface.callbacks.samples_transmitted.registerFunction(this,
    		&BILBO_CommunicationManager::_spi_txSamples_callback);
 ...
 */
void BILBO_CommunicationManager::start() {
 ...
 */
void BILBO_CommunicationManager::resetUART() {
 ...
 */
void BILBO_CommunicationManager::_uart_handleMsg_write_callback(core_comm_SerialMessage *msg) {
    ...
    if (!register_map.hasEntry(address)) {
        this->_uartResponseError(msg, BILBO_COMM_ERROR_FLAG_WRONG_ADDRESS);
        return;
    }
    ...
    if (register_map.getType(address) != REGISTER_ENTRY_TYPE_WRITABLE &&
        ...
        this->_uartResponseError(msg, BILBO_COMM_ERROR_FLAG_MSG_TYPE);
        return;
    }
    ...
    if (register_map.getInputSize(address) != msg->len) {
        ...
        // this->_uartResponseError(msg, BILBO_COMM_ERROR_FLAG_LEN);
        return;
    }
 ...
 * @param msg Pointer to the incoming serial message.
 */
void BILBO_CommunicationManager::_uart_handleMsg_read_callback(core_comm_SerialMessage *msg) {
    // Convert the message addresses to a 16-bit address.
    uint16_t address = uint8_to_uint16(msg->address_2, msg->address_3);
 ...
 * @param msg Pointer to the incoming serial message.
 */
void BILBO_CommunicationManager::_uart_handleMsg_func_callback(core_comm_SerialMessage *msg) {
    // Convert the message addresses to a 16-bit address.
    uint16_t address = uint8_to_uint16(msg->address_2, msg->address_3);
 ...
 */
void BILBO_CommunicationManager::_uartResponseError(core_comm_SerialMessage *incoming_message, uint8_t error_code) {
 ...
 */
void BILBO_CommunicationManager::_spi_rxTrajectory_callback(uint16_t len) {
    // Notify about the new trajectory.
    this->callbacks.trajectory_received.call(len);
}

void BILBO_CommunicationManager::_spi_rxPath_callback(uint16_t len) {
 ...
 */
void BILBO_CommunicationManager::_spi_txSamples_callback() {
    // No implementation for transmitting samples is provided here.
}


//void BILBO_CommunicationManager::receiveTrajectoryInputs(uint16_t steps) {
 ...
 */
void BILBO_CommunicationManager::sendMessage(BILBO_Message_t &message) {
 ...
 */
void BILBO_CommunicationManager::printf(uint8_t flag, const char *format, ...) {
    va_list args;
 ...
 */
//void BILBO_CommunicationManager::vprint(uint8_t flag, const char *format, va_list args) {
 ...
 * @param ... Additional arguments for the format string.
 */
void BILBO_CommunicationManager::send_debug(const char *format, ...) {
    va_list args;
    va_start(args, format);
 ...
 * @param ... Additional arguments for the format string.
 */
void BILBO_CommunicationManager::send_info(const char *format, ...) {
    va_list args;
    va_start(args, format);
 ...
 * @param ... Additional arguments for the format string.
 */
void BILBO_CommunicationManager::send_warning(const char *format, ...) {
    va_list args;
    va_start(args, format);
 ...
 * @param ... Additional arguments for the format string.
 */
void BILBO_CommunicationManager::send_error(const char *format, ...) {
    va_list args;
    va_start(args, format);
