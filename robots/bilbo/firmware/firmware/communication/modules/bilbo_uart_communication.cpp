 ...
 *
 * This file implements the UART communication functionality for the BILBO system.
...
static core_comm_UartInterface_config_t bilbo_communication_uart_cm4_interface_config =
{
    .uart = {
...
static const osThreadAttr_t task_attributes = {
    .name = "bilbo_uart_comm_task",           // Task name.
 ...
 * stores it in the BILBO_UART_Communication object, and starts processing UART messages.
 *
 * @param argument Pointer to the BILBO_UART_Communication instance.
 */
void bilbo_uart_comm_task(void *argument) {
    BILBO_UART_Communication *comm = (BILBO_UART_Communication*) argument;
    comm->task = xTaskGetCurrentTaskHandle();
    comm->taskFunction();
}

/**
 * @brief Default constructor for the BILBO_UART_Communication class.
 */
BILBO_UART_Communication::BILBO_UART_Communication() {
 ...
 */
void BILBO_UART_Communication::init(bilbo_uart_comm_config_t config) {
    // Initialize the UART interface using the given hardware UART and configuration.
    this->_uart_cm4.init(config.huart, bilbo_communication_uart_cm4_interface_config);

    // Register the RX callback to handle incoming data.
    this->_uart_cm4.registerCallback(
        CORE_COMM_SERIAL_SOCKET_CB_RX,
        core_utils_Callback<void, void>(this, &BILBO_UART_Communication::_rx_callback)
    );
}
 ...
 * communication tasks.
 */
void BILBO_UART_Communication::start() {
    // Start the UART interface.
    this->_uart_cm4.start();

    // Create and start the UART communication task.
    this->_thread = osThreadNew(bilbo_uart_comm_task, this, &task_attributes);
}
 ...
 */
void BILBO_UART_Communication::reset() {
 ...
 */
void BILBO_UART_Communication::send(uint8_t cmd, uint8_t module, uint16_t address, uint8_t flag, uint8_t *data, uint8_t len) {
 ...
 */
void BILBO_UART_Communication::send(core_comm_SerialMessage *msg) {
 ...
 */
void BILBO_UART_Communication::registerCallback(bilbo_uart_comm_callback_id_t callback_id,
                                                  core_utils_Callback<void, core_comm_SerialMessage*> callback) {
    switch (callback_id) {
        case BILBO_UART_COMM_CALLBACK_RX_MSG: {
            this->_callbacks.rx_msg = callback;
            break;
        }
        case BILBO_UART_COMM_CALLBACK_MSG_WRITE: {
            this->_callbacks.write_msg = callback;
            break;
        }
        case BILBO_UART_COMM_CALLBACK_MSG_READ: {
            this->_callbacks.read_msg = callback;
            break;
        }
        case BILBO_UART_COMM_CALLBACK_MSG_FUNC: {
 ...
 */
void BILBO_UART_Communication::taskFunction() {
 ...
 */
void BILBO_UART_Communication::_handleIncomingMessages() {
 ...
 */
void BILBO_UART_Communication::_rx_callback() {
    rx_available = true;
