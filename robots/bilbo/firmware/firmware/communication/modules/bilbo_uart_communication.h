 ...
 *
 *  Description:
 *      Header file for the BILBO UART Communication interface.
 *      This file defines configuration structures, constants, callback types,
 *      and the BILBO_UART_Communication class for managing UART-based communication.
 */

#ifndef COMMUNICATION_BILBO_UART_COMMUNICATION_H_
#define COMMUNICATION_BILBO_UART_COMMUNICATION_H_

#include "core.h"

// Define the UART communication queue and buffer sizes.
#define BILBO_UART_COMM_QUEUE_SIZE 10
#define BILBO_UART_COMM_BUF_SIZE 128
 ...
 */
typedef struct bilbo_uart_comm_config_t {
    UART_HandleTypeDef *huart; ///< Pointer to the UART hardware handle.
} bilbo_uart_comm_config_t;
...
typedef enum bilbo_uart_comm_callback_id_t {
    ...
    BILBO_UART_COMM_CALLBACK_MSG_FUNC, ///< Callback for function execution message events.
} bilbo_uart_comm_callback_id_t;
 ...
 */
typedef struct bilbo_uart_comm_callbacks_t {
...
} bilbo_uart_comm_callbacks_t;

//------------------------------------------------------------------------------
// BILBO_UART_Communication Class Declaration
//------------------------------------------------------------------------------
/**
 * @brief BILBO UART Communication class.
 *
 * Manages UART communication including initialization, message sending,
 * reception handling, and callback registration.
 */
class BILBO_UART_Communication {
public:
    ...
    /**
     * @brief Constructor for BILBO_UART_Communication.
     *
     * Initializes the UART communication interface.
     */
    BILBO_UART_Communication();
     ...
     */
    void init(bilbo_uart_comm_config_t config);
     ...
     */
    void registerCallback(bilbo_uart_comm_callback_id_t callback_id,
...
private:
     ...
     */
    core_comm_UartInterface<BILBO_UART_COMM_QUEUE_SIZE, BILBO_UART_COMM_BUF_SIZE> _uart_cm4;
     ...
     */
    bilbo_uart_comm_callbacks_t _callbacks;
};

#endif /* COMMUNICATION_BILBO_UART_COMMUNICATION_H_ */
