...
from core.communication.wifi.tcp_json_protocol import TCP_JSON_Message
...
class Device:
    ...
    def __init__(self, client: WebsocketServerClient, information: DeviceInformation):
        self.client = client
        self.client.callbacks.message.register(self._rxMessageCallback)
        ...
        self.logger = Logger(f"{self.information.device_id}")
    ...
    # === PRIVATE METHODS ==============================================================================================
    def _send(self, message: TCP_JSON_Message):
        if self.client.connected:
            self.client.send(asdict_optimized(message))
        else:
            self.logger.warning(f"Cannot send message to {self.address}: Client not connected")

    # ------------------------------------------------------------------------------------------------------------------
    def _rxMessageCallback(self, message: dict, client: WebsocketServerClient, *args, **kwargs):

        # Parse into a TCP JSON Message
        tcp_message = from_dict(TCP_JSON_Message, message)
        ...
        self.events.rx.set(resource=tcp_message)
    ...
    def _handleStreamMessage(self, message: TCP_JSON_Message):
        self.callbacks.stream.call(message, self)
...
class DeviceServerEvents:
    new_device: ConditionEvent = ConditionEvent(flags=[('type', str)])
    device_disconnected: ConditionEvent = ConditionEvent(flags=[('type', str)])
    stream: ConditionEvent = ConditionEvent(flags=[('type', str), ('device_id', str)])
...
class DeviceServer:
    ...
    def _clientMessageCallback(self, message: dict, client: WebsocketServerClient):
        ...
        if tcp_message.type == 'event' and tcp_message.event == 'handshake':
            ...
            self.events.new_device.set(resource=new_device, flags={'type': device_information.device_type})
    ...
    def _deviceDisconnected_callback(self, device):
        ...
        self.events.device_disconnected.set(resource=device, flags={'type': device.config.device_type})

    # ------------------------------------------------------------------------------------------------------------------
    def _deviceStream_callback(self, stream, device):
        self.callbacks.stream.call(stream, device)
        self.events.stream.set(resource=stream, flags={'type': stream.type, 'device_id': device.config.device_id})
