...
class VelocityCommand:
    v: float = 0.0       # forward velocity command [m/s] (raw)
    psi_dot: float = 0.0 # yaw rate command [rad/s] (raw)


@dataclasses.dataclass
class VelocityControllerConfig:
    # Longitudinal velocity PID (your original sign convention)
    k_p_v: float = -0.179
    k_i_v: float = -0.8
    k_d_v: float = -0.005

    # Yaw rate (psi_dot) PID -> differential torque u_psi
    ...
    # Torque saturation
    u_max: float = 5.0    # [Nm] per wheel (tune for your hardware/sim)

    # Anti-windup back-calculation gain
    k_aw: float = 1.0

    # Tilt / contact gating
    tilt_soft_deg: float = 60.0   # start reducing velocity control
    tilt_hard_deg: float = 80.0   # fully disable velocity control when |theta| > this
...
class ProjectBILBO(BILBO_DynamicAgent):
    ...
    def __init__(self, agent_id, *args, **kwargs):
        ...
        # inner balancing controller (state feedback)
        ...
        # hook into simulation scheduling
        ...
        # high-level command & controller config
        self.velocity_command = VelocityCommand()
        self.velocity_controller_config = VelocityControllerConfig()

        # controller output (for plotting/debug): wheel torques [u_l, u_r]
        self.velocity_controller_output = np.zeros(2)
        self.debug_value = 0.0

        # internally filtered / rate-limited references, used inside _velocity_control
        self._v_cmd_filtered: float = 0.0
        self._psi_dot_cmd_filtered: float = 0.0

        # slew rates (units per second)
        self._v_cmd_rate: float = 3      # [m/s^2] max change per second (tune)
        self._psi_cmd_rate: float = 6.0    # [rad/s^2] max change per second (tune)
    ...
    def input_function(self):
        """
        Read joystick and update high-level velocity command.
        No rate limiting here – that is handled in the controller so that
        CLI step commands are also subject to the same slew.
        """
        if self.joystick is None:
            return
        ...
        self.velocity_command.v = axis_forward * 2.0      # [m/s]
        self.velocity_command.psi_dot = axis_turn * 6.0   # [rad/s]

    # === UTILS ========================================================================================================
    def _slew(self, curr: float, target: float, max_rate: float) -> float:
        """
    ...
    def _controller(self) -> BILBO_3D_Input:
        """
        Runs before dynamics every time step.
        Combines inner state feedback with optional outer velocity/yaw loops.
        """
        if self.mode == BILBO_Control_Mode.OFF:
            ...
            # pure regulation to upright (no velocity loop)
            controller_input = self.input.asarray() - self.K @ self.dynamics.state.asarray()

        elif self.mode == BILBO_Control_Mode.VELOCITY:
            # outer velocity/yaw PID -> wheel torques (forward + differential)
            u_l, u_r = self._velocity_control()
            self.velocity_controller_output = np.asarray([u_l, u_r])
            controller_input = np.asarray([u_l, u_r]) - self.K @ self.dynamics.state.asarray()
    ...
    # === OUTER VELOCITY & YAW PID (TORQUE-LEVEL, WITH INTERNAL SLEW) =================================================
    def _velocity_control(self) -> tuple[float, float]:
        """
        VELOCITY mode:

        - Internal slew on velocity_command.v and psi_dot, so any source
          (joystick, CLI step, etc.) is rate-limited.
        - PID on forward velocity v -> forward torque u_v
        - PID on yaw rate psi_dot -> differential torque u_psi
        - Combined into left/right torques:
              u_l = u_v - u_psi
              u_r = u_v + u_psi
        - Includes:
              * tilt/contact gating
              * torque saturation
              * simple anti-windup back-calculation
        """
        ...
        ground_contact = getattr(self, "_ground_contact", False)

        if abs(theta_deg) <= cfg.tilt_soft_deg:
            gain_scale = 1.0
        elif abs(theta_deg) >= cfg.tilt_hard_deg:
            gain_scale = 0.0
        else:
            gain_scale = (cfg.tilt_hard_deg - abs(theta_deg)) / (cfg.tilt_hard_deg - cfg.tilt_soft_deg)

        if ground_contact:
            gain_scale = 0.0

        # if fully disabled, zero outputs and reset integrators (avoid windup on the floor)
        if gain_scale == 0.0:
            self._v_integral = 0.0
            self._psi_dot_integral = 0.0
            self._v_last_error = 0.0
            self._psi_dot_last_error = 0.0
            # also reset internal filtered refs so they restart clean
            self._v_cmd_filtered = 0.0
            self._psi_dot_cmd_filtered = 0.0
            return 0.0, 0.0

        # --- internal slew: apply to desired commands (common for joystick & CLI) ---
        self._v_cmd_filtered = self._slew(
            self._v_cmd_filtered,
            self.velocity_command.v,
            self._v_cmd_rate
        )
        self._psi_dot_cmd_filtered = self._slew(
            self._psi_dot_cmd_filtered,
            self.velocity_command.psi_dot,
            self._psi_cmd_rate
        )

        v_ref = self._v_cmd_filtered
        psi_ref = self._psi_dot_cmd_filtered

        # --- errors ---
        e_v = v_ref - v
        e_psi_dot = psi_ref - psi_dot
        ...
        # --- saturate integrals ---
        if self._v_integral > cfg.k_integral_max_v:
            self._v_integral = cfg.k_integral_max_v
        elif self._v_integral < -cfg.k_integral_max_v:
            self._v_integral = -cfg.k_integral_max_v

        if self._psi_dot_integral > cfg.k_integral_max_psi_dot:
            self._psi_dot_integral = cfg.k_integral_max_psi_dot
        elif self._psi_dot_integral < -cfg.k_integral_max_psi_dot:
            self._psi_dot_integral = -cfg.k_integral_max_psi_dot

        # --- PID for v -> forward torque u_v ---
        u_v = (
            cfg.k_p_v * e_v +
            cfg.k_i_v * self._v_integral +
            cfg.k_d_v * e_v_dot
        )

        # --- PID for psi_dot -> differential torque u_psi ---
        u_psi = (
            cfg.k_p_psi_dot * e_psi_dot +
            cfg.k_i_psi_dot * self._psi_dot_integral +
            cfg.k_d_psi_dot * e_psi_dot_dot
        )

        # apply tilt gain scaling
        u_v *= gain_scale
        u_psi *= gain_scale

        # combine into wheel torques (unsaturated)
        u_l = u_v - u_psi
        u_r = u_v + u_psi

        # --- torque saturation ---
        u_max = cfg.u_max
        u_l_sat = float(np.clip(u_l, -u_max, u_max))
        u_r_sat = float(np.clip(u_r, -u_max, u_max))

        # --- simple anti-windup back-calculation ---
        du_l = u_l_sat - u_l
        du_r = u_r_sat - u_r

        # decompose back into forward/differential "lost" torque
        du_v = 0.5 * (du_l + du_r)
        du_psi = 0.5 * (du_r - du_l)

        # back-calc: adjust integrals so they don't keep pushing into saturation
        k_aw = cfg.k_aw
        self._v_integral += k_aw * du_v * self.Ts
        self._psi_dot_integral += k_aw * du_psi * self.Ts

        self.debug_value = u_psi  # or anything else useful

        return u_l_sat, u_r_sat
    ...
    def _set_velocity(self, v: float, psi_dot: float):
        """
        CLI hook: set desired velocity/yaw rate.
        These are still subject to the internal slew in _velocity_control.
        """
    ...
    def reset(self, x0=None):
        ...
        self.velocity_command.v = 0.0
        self.velocity_command.psi_dot = 0.0
        ...
        # Reset internal filtered commands
        self._v_cmd_filtered = 0.0
        self._psi_dot_cmd_filtered = 0.0
    ...
    def _set_velocity_pid(self, kp: float, ki: float, kd: float):
        """
        Set PID gains for the forward velocity v (torque-level PID).
        psi_dot PID gains are configured separately (in code).
        """
    ...
    def _set_control_mode(self, mode: int):
        ...
        if mode == 3:
            # VELOCITY mode: reset PID states & filtered commands
            self._v_integral = 0.0
            self._v_last_error = 0.0
            self._psi_dot_integral = 0.0
            self._psi_dot_last_error = 0.0
    ...
    def _set_state(self, x: float | None = None, y: float | None = None,
                   theta: float | None = None, psi: float | None = None):
