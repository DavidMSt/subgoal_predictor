...
from robots.bilbo.simulation.bilbo_model import BILBO_Dynamics_2D, DEFAULT_BILBO_MODEL, \
    BILBO_Dynamics_2D_Linear, BILBO_2D_POLES, BILBO_MICHAEL_MODEL, BilboModel
...
def main():
    ...
    # bilbo_dynamics.K = [0, -0.25, -0.2, -0.03]
    # bilbo_dynamics.K = [0, -0.25, -0.2, -0.03]
    print(K)
    ...
    theta_dot = [state.theta_dot for state in states]
    theta_dot = np.array(theta_dot) + normal(0, noise_std, len(theta_dot))

    noise_v_std = 0.005
    noise_v = normal(0, noise_v_std, len(theta))
    v = np.array(v) + noise_v

    plt.plot(theta_dot, label='theta_dot')
    plt.plot(v, label='v')
