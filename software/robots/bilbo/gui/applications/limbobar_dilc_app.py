...
class LimboBar_DILC_APP(GUI_Popup_Application):
    experiment: LimboBar_DILC_Experiment

    def __init__(self, gui, robot: BILBO, experiment: LimboBar_DILC_Experiment, config: dict = None):
        super().__init__('limbobar_dilc_app', 'LimboBar DILC APP', config)
        self.gui = gui
        self.robot = robot
        self.experiment = experiment
        self._trial_rows = []

        exp_id = experiment.settings.id if experiment.settings else 'LimboBar DILC'
        limbo_height = experiment.settings.limbo_bar.height if experiment.settings else '?'
        self.popup = Popup(f"{self.id}_popup",
                           title=f'LimboBar DILC -- {exp_id} (h={limbo_height}m)',
    ...
    def _update_hits_counter(self):
        ...
        total_hits = sum(1 for t in trials if t.limbo_bar_hit)
        total = len(trials)
    ...
    def _update_best_trial(self):
        ...
        hit_str = " (HIT)" if best.limbo_bar_hit else ""
        self.status_widget.elements['best'].status = f'Trial {best.trial_index + 1}  ({best.e_norm_ilc:.6f}){hit_str}'
    ...
    def _plotOutputData(self):
        ...
        for i, trial in enumerate(trials):
            ...
            is_latest = (i == n - 1)
            color = 'red' if trial.limbo_bar_hit else colors[i % max(1, len(colors))]
            linestyle = '--' if trial.limbo_bar_hit else '-'
            label_parts = []
            if is_latest:
    ...
    def _plotErrorNorms(self):
        ...
        if trials:
            ...
            # Mark hits with X
            hit_x = [i + 1 for i, t in enumerate(trials) if t.limbo_bar_hit]
            hit_y = [t.e_norm_ilc for t in trials if t.limbo_bar_hit]
            if hit_x:
                ax.scatter(hit_x, hit_y, s=100, color="red", marker='x', linewidths=2.0, zorder=3, label="Hit")
