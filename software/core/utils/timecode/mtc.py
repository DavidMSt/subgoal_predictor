...
# Timeout for subprocess-based MIDI availability check (seconds)
MIDI_CHECK_TIMEOUT = 5.0


def _midi_check_worker(result_queue: multiprocessing.Queue, port_match: str) -> None:
    """
    Worker function that runs in a subprocess to safely check MIDI availability.
    If RtMidi/CoreMIDI crashes, only this subprocess dies.
    """
    try:
        import mido
        ...
        for name in ports:
            if port_match.lower() in name.lower():
                matching_port = name
                break
        result_queue.put(("success", matching_port, list(ports)))
    except Exception as e:
        result_queue.put(("error", str(e), []))


def safe_check_midi_available(port_match: str = PORT_MATCH, timeout: float = MIDI_CHECK_TIMEOUT) -> tuple[bool, str | None, list[str]]:
    """
    Check if MIDI is available by running the check in a subprocess.

    This protects against the RtMidi/CoreMIDI crash (error -304) that can occur on macOS
    when the MIDI subsystem is in a bad state. If the subprocess crashes, the main
    process survives and we return False.

    Returns:
        Tuple of (success: bool, matching_port: str | None, all_ports: list[str])
        - success: True if MIDI subsystem is available
        - matching_port: Name of port matching port_match, or None
        - all_ports: List of all available MIDI input port names
    """
    result_queue: multiprocessing.Queue = multiprocessing.Queue()

    # Use 'spawn' context to ensure clean subprocess without inherited state
    ...
    if proc.exitcode != 0:
        # Subprocess crashed (likely the RtMidi/CoreMIDI error)
        return (False, None, [])

    try:
        status, port_or_error, ports = result_queue.get_nowait()
        if status == "success":
            return (True, port_or_error, ports)
        else:
            # Python exception in subprocess
            return (False, None, [])
    except Exception:
        return (False, None, [])
...
class MTC_Decoder:
    ...
    def start(self) -> bool:
        ...
        Returns False immediately if no Tentacle port can be found or if MIDI is unavailable.
        """
        # First, check MIDI availability in a subprocess to protect against CoreMIDI crashes
        self.logger.info("Checking MIDI availability (subprocess safety check)...")
        midi_available, port_name, all_ports = safe_check_midi_available(PORT_MATCH)

        if not midi_available:
            self.logger.error(
                "MIDI subsystem is unavailable (CoreMIDI initialization failed). "
                "This can happen when the MIDI system is in a bad state. "
                "Try restarting any MIDI applications or rebooting."
            )
            return False

        if all_ports:
            self.logger.debug(f"Available MIDI ports: {all_ports}")

        if port_name is None:
            self.logger.warning(f"Could not find a MIDI input containing '{PORT_MATCH}'")
            return False
