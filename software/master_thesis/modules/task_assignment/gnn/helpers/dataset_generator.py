...
class CostDatasetGenerator:
    ...
    def _clear_previous_datasets(self, out_path: Path):
          for f in out_path.glob('*.pt'):
            if f.stem.startswith('dataset_n'):
                f.unlink()
            else:
                self.logger.warning(f'Skipping non-dataset .pt file: {f}')

    def generate_dataset(self,
            team_sizes: list[int] = [5, 10, 15, 20],
            samples_per_size: int = 1000,
            seed: int = 42,
            out_dir: str = 'master_thesis/modules/task_assignment/gnn/datasets',
    ) -> None:
        
        #clean previously created data in directory
        out_path = Path(out_dir)
        out_path.mkdir(parents=True, exist_ok=True)
        self._clear_previous_datasets(out_path=out_path)

        # loop through each team size, create sub-dataset file for each size
        for n in team_sizes:

            # initialize input-cost tensor
            sub_dataset_cost = torch.empty(samples_per_size, n, n)
            # initialize output-label tensor
            sub_dataset_1h_label = torch.zeros(samples_per_size, n, n)

            g = torch.Generator().manual_seed(seed + n) # different seed per team size
            for sample in range(samples_per_size):

                # randomly select cost values
                cost_matrix = torch.rand(size=[n, n], generator=g)
                sub_dataset_cost[sample,:,:] = cost_matrix

                # determine the optimal assignment and save in n x n tensor
                label_matrix = torch.zeros(n,n)
                row_ind, col_ind = linear_sum_assignment(cost_matrix.numpy())
                label_matrix[row_ind, col_ind] = 1.0
                sub_dataset_1h_label[sample, :, :] = label_matrix

            # write to file
            file_path = out_path / f"dataset_n{n:02d}.pt"
            torch.save({
                'cost': sub_dataset_cost,
                'labels': sub_dataset_1h_label,
                'team_size': n,
...
# class DatasetPositionGenerator:
    ...
    # def _sample_poses_in_limits(self, n: int) -> np.ndarray:
    ...
    # def _compute_squared_euclidean_cost_matrix(self, agents: np.ndarray, tasks: np.ndarray) -> np.ndarray:
    #     """Compute (squared) Euclidean distance cost matrix."""
    #     agent_xy = agents[:, :2]
    #     task_xy = tasks[:, :2]
    #     diff = agent_xy[:, None, :] - task_xy[None, :, :]
    #     return np.linalg.norm(diff, axis=2)**2
    ...
    #     agent_poses = self._sample_poses_in_limits(n)
    #     task_poses = self._sample_poses_in_limits(n)

    #     cost_matrix = self._compute_squared_euclidean_cost_matrix(agent_poses, task_poses)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--team-sizes', type=int, nargs='+', default=[10],
    ...
    generator = CostDatasetGenerator(log_level='INFO')
    generator.generate_dataset(
        team_sizes=args.team_sizes,
        samples_per_size=args.n_samples,
